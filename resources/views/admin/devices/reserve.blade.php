    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="reserve_device_{{$system->id}}_modal">Add device to system</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form method="post" action="{{ route('admin.device.reserve') }}">
                    @csrf
                    <input type="hidden" name="system_id" value="{{ $system->id }}">
                    <div class="mb-3">
                        <label for="user-checkbox">Select device that you want to add to system:</label>
